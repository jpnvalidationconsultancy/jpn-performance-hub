...
function syncSettings(){return getStore("syncSettings",{apiBase:"",syncKey:"",auto:false,lastSync:null})}
function syncEndpoint(){const cfg=syncSettings();const base=(cfg.apiBase||"").trim().replace(/\/$/,"");return base?`${base}/api/hub-data`:"/api/hub-data"}
function hasSyncConfig(){const cfg=syncSettings();return !!cfg.syncKey}
function setSyncStatus(message,good=true){const box=document.getElementById("syncStatus");if(box)box.innerHTML=`<p class='${good?"score-good":"score-bad"}'>${message}</p>`}
function mergeBy(items,keyFn){const map=new Map();items.forEach(item=>{const key=keyFn(item);if(!map.has(key))map.set(key,item)});return Array.from(map.values())}
function foodKey(f){return[f.date,f.meal,f.name,f.qty,f.calories,f.protein,f.carbs,f.fat].map(v=>String(v||"").trim().toLowerCase()).join("|")}
function metricKey(m){return String(m.date||"").trim()||JSON.stringify(m)}
function mergeCloudState(cloud={}){const merged={};merged.metrics=mergeBy([...(cloud.metrics||[]),...metrics()],metricKey).sort((a,b)=>(b.date||"").localeCompare(a.date||""));merged.foods=mergeBy([...(cloud.foods||[]),...foods()],foodKey).sort((a,b)=>(b.date||"").localeCompare(a.date||""));merged.customFoodDb={...(cloud.customFoodDb||{}),...getStore("customFoodDb",{})};return merged}
function applySyncState(state){if(Array.isArray(state.metrics))setStore("metrics",state.metrics);if(Array.isArray(state.foods))setStore("foods",state.foods);if(state.customFoodDb&&typeof state.customFoodDb==="object")setStore("customFoodDb",state.customFoodDb);loadCustomFoodDb();renderAll()}
async function readCloudState(){const cfg=syncSettings();const res=await fetch(syncEndpoint(),{headers:{"X-Hub-Sync-Key":cfg.syncKey},cache:"no-store"});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||"Cloud read failed");return data.state||{}}
async function writeCloudState(state){const cfg=syncSettings();const res=await fetch(syncEndpoint(),{method:"POST",headers:{"Content-Type":"application/json","X-Hub-Sync-Key":cfg.syncKey},body:JSON.stringify({state})});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||"Cloud write failed");return data}
function saveSyncSettings(){const next={apiBase:syncApiBase.value.trim(),syncKey:syncKey.value.trim(),auto:!!syncAuto.checked,lastSync:syncSettings().lastSync};setStore("syncSettings",next);setSyncStatus("Sync settings saved.")}
async function syncFromCloud(){try{if(!hasSyncConfig()){setSyncStatus("Add your sync key first.",false);return}setSyncStatus("Pulling food and Garmin data from cloud...");const cloud=await readCloudState();const merged=mergeCloudState(cloud);applySyncState(merged);setStore("syncSettings",{...syncSettings(),lastSync:new Date().toISOString()});setSyncStatus(`Pulled and merged ${merged.metrics.length} Garmin rows and ${merged.foods.length} food rows.`)}catch(e){setSyncStatus(e.message,false)}}
async function syncToCloud(){try{if(!hasSyncConfig()){setSyncStatus("Add your sync key first.",false);return}setSyncStatus("Pushing food and Garmin data to cloud...");const cloud=await readCloudState().catch(()=>({}));const merged=mergeCloudState(cloud);await writeCloudState(merged);applySyncState(merged);setStore("syncSettings",{...syncSettings(),lastSync:new Date().toISOString()});setSyncStatus(`Pushed ${merged.metrics.length} Garmin rows and ${merged.foods.length} food rows.`)}catch(e){setSyncStatus(e.message,false)}}
async function syncNow(){await syncToCloud()}
function autoCloudSync(){const cfg=syncSettings();if(!cfg.auto||!cfg.syncKey)return;syncToCloud()}
...
function renderSettings(){const s=settings();setWeight.value=s.weight;setBodyFat.value=s.bodyFat;setProtein.value=s.proteinGkg;setRunLimit.value=s.runLimit;setHydration.value=s.hydrationTarget;const cfg=syncSettings();if(window.syncApiBase){syncApiBase.value=cfg.apiBase||"";syncKey.value=cfg.syncKey||"";syncAuto.checked=!!cfg.auto;if(cfg.lastSync)setSyncStatus(`Last synced ${new Date(cfg.lastSync).toLocaleString()}`)}}

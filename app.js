...
function autoEstimateFood(){const name=foodName.value.toLowerCase();const match=Object.keys(FOOD_DB).find(k=>name.includes(k));if(!match){alert("No estimate found. Enter calories/macros manually.");return}const item=FOOD_DB[match];foodCalories.value=Math.round(item.cal);foodProtein.value=Math.round(item.p);foodCarbs.value=Math.round(item.c);foodFat.value=Math.round(item.f);foodQty.value=foodQty.value||item.unit}
